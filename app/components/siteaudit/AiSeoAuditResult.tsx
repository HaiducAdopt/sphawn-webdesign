...
export default function AiSeoAuditResult({ data }: Props) {
  const aiReadinessScore =
    data.scores?.aiReadiness ?? data.score;
      ...
      <div className="mt-6 grid gap-4 md:grid-cols-[0.5fr_1fr]">
        ...
        <div className="rounded-2xl bg-black/25 p-5">
          ...
          <div className="mt-4 grid gap-3 text-sm text-white/70 md:grid-cols-2">
            ...
            <p>
              FAQ detected on this page:{" "}
              {data.extracted.faqDetected ? "Yes" : "No"}
            </p>
            <p>
              Open Graph:{" "}
              {data.extracted.ogDetected ? "Yes" : "No"}
            </p>
            <p>
              Canonical:{" "}
              {data.extracted.canonicalDetected ? "Yes" : "No"}
            </p>
          </div>
        </div>
      </div>
