...
export default function TailwindRelatedArticles() {
  const t = useTranslations("TailwindPage.relatedArticles");
        ...
        <div className="mt-12 grid gap-6 lg:grid-cols-3">
          {articles.map((article) => (
            <article key={article.slug} className="flex flex-col rounded-3xl border border-white/10 bg-[#07111f] p-7">
              ...
              <Link href={`/${locale}/lab/${article.slug}`} className="mt-7 inline-flex items-center gap-2 font-semibold text-cyan-300 transition hover:text-cyan-200">
